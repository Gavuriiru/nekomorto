...
describe("ImageLibraryDialog alt text editor", () => {
  beforeEach(() => {
  ...
  it("abre a edicao de texto alternativo para uploads e salva no endpoint dedicado", async () => {
    ...
    const uploadButton = await screen.findByRole("button", { name: /upload-1\.png/i });
    fireEvent.contextMenu(uploadButton);

    fireEvent.click(await screen.findByText("Editar texto alternativo"));

    const dialogs = await screen.findAllByRole("dialog");
    const altDialog = dialogs[dialogs.length - 1];
    const input = within(altDialog).getByLabelText("Texto alternativo");
    expect(input).toHaveValue("Alt anterior");

    fireEvent.change(input, { target: { value: "Alt atualizado" } });
    fireEvent.click(within(altDialog).getByRole("button", { name: "Salvar" }));

    await waitFor(() => {
      const patchCall = apiFetchMock.mock.calls.find(
        (call) => String(call[1] || "") === "/api/uploads/upload-1/alt-text",
      );
    ...
    const payload = JSON.parse(String(request.body || "{}")) as { altText?: string };

    expect(payload.altText).toBe("Alt atualizado");
  });
});
